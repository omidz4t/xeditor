...
props.icon = '💡'
  ...
  if (type === 'poll') {
    if (!Array.isArray(props.pollOptions) || props.pollOptions.length === 0) {
      props.pollOptions = [
        { id: generateBlockId(), text: '', votes: [] },
        { id: generateBlockId(), text: '', votes: [] },
      ]
    } else {
      props.pollOptions = props.pollOptions.map((opt) => ({
        id: opt.id || generateBlockId(),
        text: typeof opt.text === 'string' ? opt.text : '',
        votes: Array.isArray(opt.votes) ? opt.votes.filter((v) => typeof v === 'string') : [],
      }))
    }
    if (props.pollAllowMultiple === undefined) {
      props.pollAllowMultiple = false
    }
    if (props.pollClosed === undefined) {
      props.pollClosed = false
    }
  }
...
export function detectDir(text: string): 'rtl' | 'ltr' {
  for (const ch of text) {
    // First strong RTL character (Hebrew / Arabic / Farsi / …) wins.
    if (RTL_CHAR.test(ch)) {
      return 'rtl'
    }

    // First strong LTR letter (Latin, etc.) wins.
    // Use Unicode letter class so non-English LTR scripts still count.
    if (/\p{L}/u.test(ch) && !RTL_CHAR.test(ch)) {
      return 'ltr'
    }
  }
...
/**
 * Resolve layout direction for a block (gutter, markers, typing).
 * @param previousDir Direction of the block above — used when this block is empty
 *   so a new line under RTL content stays RTL until the user types LTR.
 */
export function resolveBlockDirection(
  ...
  // Manual toolbar override always wins while set.
  if (block.props.dirManual && (explicit === 'rtl' || explicit === 'ltr')) {
    return explicit
  }

  // Prefer live detection from content so dir tracks language as the user types
  // (including after deleting RTL text and typing English).
  ...
  // Empty / no content: inherit the line above, then editor default.
  if (previousDir === 'rtl' || previousDir === 'ltr') {
    return previousDir
  }
...
/** Map each block id → resolved dir, carrying forward empty lines from the previous block. */
export function buildBlockDirectionMap(
  blocks: Block[],
  editorDir: 'ltr' | 'rtl' = 'ltr',
): Map<string, 'ltr' | 'rtl'> {
  const map = new Map<string, 'ltr' | 'rtl'>()
  let prev: 'ltr' | 'rtl' = editorDir
  for (const block of blocks) {
    const dir = resolveBlockDirection(block, editorDir, prev)
    map.set(block.id, dir)
    prev = dir
  }
  return map
}
