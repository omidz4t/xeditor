...
return ''
  ...
  if (block.type === 'page') {
    return block.props.pageTitle?.trim() || 'Untitled'
  }

  if (block.type === 'poll') {
    const question = spansToText(block.content).trim() || 'Poll'
    const options = (block.props.pollOptions ?? [])
      .map((opt) => `- ${opt.text || 'Option'} (${opt.votes?.length ?? 0})`)
      .join('\n')
...
export function extractHeadings(blocks: Block[]): DocHeading[] {
  ...
  for (const block of blocks) {
    const match = /^heading_([1-6])$/.exec(block.type)
